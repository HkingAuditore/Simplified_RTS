...
public class FindEnemyTrigger : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (isRangeUnit)
        {
            ...
            this.GetComponent<SphereCollider>().radius = ((RangedAttackUnit) _baseUnit).AttackRange;
        }
        else
        {
            _baseUnit = parent.GetComponent<MeleeUnit>();
            this.GetComponent<SphereCollider>().radius = ((MeleeUnit) _baseUnit).findEnemyRadius;

        }
        
        _thisRoadTag = parent.tag;
        _enemyLayerMask = 1 << LayerMask.NameToLayer(LayerMask.LayerToName(parent.gameObject.layer) == "ASide" ? "BSide" : "ASide");
    ...
    {
        if (!other.isTrigger && !other.gameObject.CompareTag("Untagged") && (((1 << other.gameObject.layer) & _enemyLayerMask.value) != 0))
        {
            ...
            if (other.gameObject.TryGetComponent(typeof(Unit), out unitComponent))
            {
                // Debug.Log( _baseUnit.gameObject.name + " New Enemy In Layer!");
                        ...
                        other.gameObject.CompareTag("Door"))
                    {
                        ...
                        Debug.Log( _baseUnit.gameObject.name + " New Enemy In!:: " + other.gameObject.GetComponent<Unit>().gameObject.name);
                    }
            }
        }
    }
